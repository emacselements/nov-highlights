...
(defun nov-highlights--current-book-id ()
  "Return a stable book id for the current Nov buffer without touching history."
  (let ((nov-buf (or (and (derived-mode-p 'nov-mode) (current-buffer))
                     (buffer-base-buffer))))
    (unless (and nov-buf (buffer-live-p nov-buf))
      (user-error "Run this command from a Nov buffer (an EPUB)"))
    (with-current-buffer nov-buf
      ;; nov-file-name is the full path to the .epub; safest stable key.
      (or (bound-and-true-p nov-file-name)
          (user-error "Cannot determine book id: nov-file-name is missing")))))
